...
impl Data for F32 {
    ...
    data_methods!(sum, 1, "dim");
    data_methods!(max, 2, "dim");
    data_methods!(min, 3, "dim");
    ...
    fn conv2d(&self, _kernel: &Self, _params: &crate::array::data::ParamsConv2D) -> WResult<Self> {
        todo!()
    }

    fn flipped(&self, _dims: &[usize], _flip: &[usize]) -> WResult<Self> {
        todo!()
    }

    fn conv_transpose2d(&self, _kernel: &Self, _params: &crate::array::data::ParamsConvTranspose2D) -> WResult<Self> {
        
        todo!()
    }

    fn gather(&self, _indexes: &Self, _dims_self: &[usize], _dims_ids: &[usize], _dim: usize) -> WResult<Self> {
        todo!()
    }
...
impl PartialEq for F32 {
    fn eq(&self, other: &Self) -> bool {
        let a = self.to_vec().unwrap();
        let b = other.to_vec().unwrap();
        a.iter().zip(b.iter()).all(| (aa, bb) | (aa - bb).abs() < 0.0001)
    }
}
