...
__global__ void op_map_dim(float* A, float* C, const size_t N, const size_t dim0, const size_t n0, const size_t n1, const u_int8_t op) {
    ...
    const size_t n0_temp = id / n1;
    const size_t n1_temp = id % n1;
...
__global__ void op_broadcast(float* A, float* C, const size_t N, const size_t n, const size_t n0, const size_t n1) {
    ...
    const size_t a = n0_temp * n1 + n1_temp;
    const size_t i_start0 = n0_temp * n * n1 + n1_temp;
...
__global__ void op_matmul_nd(float* A, float* B, float* C, const size_t N, const size_t n, const size_t n0, const size_t n1, const size_t pre) {
    ...
    const size_t t = n0 * n1;
    const size_t pre_temp = id / t;
    const size_t pre_temp_left = id % t;
    const size_t n0_temp = pre_temp_left / n1;
    const size_t n1_temp = pre_temp_left % n1;
    
    size_t start1 = pre_temp * n0 * n + n0_temp * n;
...
extern "C" {
    ...
    F32* wdata_matmul_1d(const F32* lhs, const F32* rhs) {
        float* data1 = wdata_get_data_all(lhs);
        float* data2 = wdata_get_data_all(rhs);

        float data_res = 0.0;
        for(size_t i = 0; i < lhs->len; i++) {
            data_res += data1[i] * data2[i];
        }
        free(data1);
        free(data2);
