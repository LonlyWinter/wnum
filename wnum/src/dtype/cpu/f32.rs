...
use crate::array::{data::{Data, ParamsConv2D, ParamsConvTranspose2D}, error::{WResult, WError}};
...
macro_rules! data_methods {
    ...
    ($method:ident, $ty:ty, "tofloat") => {
        fn $method(&self) -> WResult<Self> {
            self.map_item(| v | (*v as $ty).$method() as $ty)
        }
    };
...
impl F32Test {
    ...
    pub fn map_zip_item<F: Fn((&f32, &f32)) -> f32>(&self, rhs: &Self, f: F) -> WResult<Self> {
        let res = self.0.iter().zip(rhs.0.iter()).map(f).collect::<Vec<_>>();
        Self::new(res)
    }
}

impl Deref for F32Test {
    type Target = Vec<f32>;
...
impl Data for F32Test {
    ...
    // basic
    // broadcast
    data_methods!(add, broadcast_add, +, f32);
    data_methods!(sub, broadcast_sub, -, f32);
    data_methods!(div, broadcast_div, /, f32);
    data_methods!(mul, broadcast_mul, *, f32);
    ...
    // https://github.com/huggingface/candle/blob/main/candle-core/src/cpu_backend/mod.rs#L451
    fn gather(&self, indexes: &Self, dims_self: &[usize], dims_ids: &[usize], dim: usize) -> WResult<Self> {
        let dst_len: usize = dims_ids.iter().product();
        let dst_left_len: usize = dims_ids[..dim].iter().product();
        let dst_dim_len = dims_ids[dim];
        let dst_right_len: usize = dims_ids[dim + 1..].iter().product();

        let src_dim_len = dims_self[dim];
        let src_right_len: usize = dims_self[dim + 1..].iter().product();

        let mut dst = vec![0 as f32; dst_len];
        for left_i in 0..dst_left_len {
            ...
            for i in 0..dst_dim_len {
                ...
                for right_i in 0..dst_right_len {
                    ...
                    let index = indexes.0[dst_idx] as usize;
                    if index >= src_dim_len {
                        return Err(WError::DimNumError(format!("Dim {} {} not found", index, src_dim_len)))
                    }
                    let src_idx = start_src_idx + index * src_right_len + right_i;
                    dst[dst_idx] = self.0[src_idx]
                }
            }
        }
        Self::from_vec(dst)
    ...
    // https://github.com/huggingface/candle/blob/main/candle-core/src/cpu_backend/mod.rs#L604
    fn scatter(&self, indexes: &Self, src: &Self, dims_self: &[usize], dims_ids: &[usize], dim: usize) -> WResult<Self> {
        let dst_dim_len = dims_self[dim];
        let dst_right_len: usize = dims_self[dim + 1..].iter().product();
        let dst_len = dims_self.iter().product();

        let ids_left_len: usize = dims_ids[..dim].iter().product();
        let ids_dim_len = dims_ids[dim];
        let ids_right_len: usize = dims_ids[dim + 1..].iter().product();
        ...
        for left_i in 0..ids_left_len {
            ...
            for i in 0..ids_dim_len {
                ...
                for right_i in 0..dst_right_len {
                    ...
                    let index = indexes.0[ids_idx] as usize;
                    if index >= dst_dim_len {
                        return Err(WError::DimNumError(format!("Dim {} {} not found", index, dst_dim_len)))
                    }
                    let dst_idx = start_dst_idx + index * dst_right_len + right_i;
                    dst[dst_idx] = src.0[ids_idx];
                }
            }
        }
        Self::from_vec(dst)
