...
fn test_tensor_max() -> WResult<()> {
    let data = vec![vec![vec![3.0, 1.0, 4.0], vec![1.0, 5.0, 9.0]], vec![vec![2.0, 1.0, 7.0], vec![8.0, 2.0, 8.0]]];
    let data: WTensor<F32> = WTensor::from_vec3(data)?;

    let t0_real = WTensor::from_vec3(vec![vec![vec![4.0, ], vec![9.0, ]], vec![vec![7.0, ], vec![8.0, ]]])?;
    let t1_real = WTensor::from_vec3(vec![vec![vec![4.0, 4.0, 4.0], vec![9.0, 9.0, 9.0]], vec![vec![7.0, 7.0, 7.0], vec![8.0, 8.0, 8.0]]])?;
...
fn test_tensor_sum() -> WResult<()> {
    let data = vec![vec![vec![3.0f32, 1.0, 4.0], vec![1.0, 5.0, 9.0]], vec![vec![2.0, 1.0, 7.0], vec![8.0, 2.0, 8.0]]];
    let data: WTensor<F32> = WTensor::from_vec3(data)?;

    let t0_real = WTensor::from_vec3(vec![vec![vec![8.0, ], vec![15.0, ]], vec![vec![10.0, ], vec![18.0, ]]])?;
    let t1_real = WTensor::from_vec3(vec![vec![vec![8.0, 8.0, 8.0], vec![15.0, 15.0, 15.0]], vec![vec![10.0, 10.0, 10.0], vec![18.0, 18.0, 18.0]]])?;
...
fn test_tensor_exp() -> WResult<()> {
    ...
    let x2: WTensor<F32> = WTensor::from_vec2(vec![vec![2.71828183,7.38905610,20.08553692],vec![54.59815003,148.41315910,403.42879349],vec![1096.63315843,2980.95798704,8103.08392758],vec![22026.46579481,59874.14171520,162754.79141900]])?;

    let t = x1.exp()?.to_vec()?;
    let x2 = x2.to_vec()?;

    // cuda exp
    let t = t.into_iter().zip(x2).all(| (a, b) | (a - b).abs() < 0.1);
    assert!(t, "exp");
    Ok(())
}
...
fn test_grad_mix2() -> WResult<()> {
    ...
    let y_real = WArr::from_vec1(vec![2.0986, 1.0, 2.3863, -0.8971])?.round(3.0)?;
    ...
    let y = y.read_data()?.round(2.0)?;
    #[allow(clippy::approx_constant)]
    let y_real = WArr::from_vec1(vec![20.0855, 2.7183, 54.5982, 1.1618])?.round(2.0)?;
    #[allow(clippy::approx_constant)]
    ...
    let y = y.read_data()?.round(2.0)?;
    let y_real = WArr::from_vec1(vec![403.429, 7.389, 2980.958, 1.35])?.round(2.0)?;
    let grad_x_real = WArr::from_vec1(vec![806.86, 14.78, 5961.92, 2.7])?.round(2.0)?;
...
#[test]
fn test_big_matmul() -> WResult<()> {
    let n0 = 2048;
    let n1 = 768;
    let n = 2048;

    let a: WTensor<F32> = WTensor::random_normal((n0, n))?;
    let b = WTensor::random_normal((n, n1))?;

    let c = a.matmul(&b)?;

    let dim = WArrDims::from((n0, n1));
    let len = dim.dims_len();
    let dim_res = c.dims()?;
    let len_res = c.to_vec()?.len();

    assert_eq!(dim, dim_res, "big matmul dim");
    assert_eq!(len, len_res, "big matmul len");
